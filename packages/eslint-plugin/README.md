...
| [bad-min-max-func](docs/rules/bad-min-max-func.md)                                 | Disallow inverted nested Math.min/Math.max clamps that always return a constant bound                                           | 🌐 🏆 ✅ |     |     |
...
| [missing-throw](docs/rules/missing-throw.md)                                       | Disallow standalone new Error expressions that likely forgot the throw keyword                                                  | 🌐 🏆 ✅ | 🔧  |     |
...
| [no-async-promise-executor](docs/rules/no-async-promise-executor.md)               | Disallow async Promise executor functions because thrown errors are not captured by the outer Promise                           | 🌐 🏆 ✅ |     |     |
| [no-chained-type-assertions](docs/rules/no-chained-type-assertions.md)             | Disallow chained TypeScript type assertions that discard type evidence                                                          | 🌐 ✅ ⌨️ |     |     |
...
| [no-debug-scaffolding](docs/rules/no-debug-scaffolding.md)                         | Disallow temporary debugger statements and console debug scaffolding left behind during development                             | 🌐 🧹 ✅ |     |     |
| [no-dynamic-code-execution](docs/rules/no-dynamic-code-execution.md)               | Disallow dynamic code execution through eval, Function constructors, and string-based timers                                    | 🌐 🏆 ✅ |     |     |
...
| [no-floating-promise](docs/rules/no-floating-promise.md)                           | Disallow calling async functions or Promise-returning expressions without awaiting, returning, or explicitly voiding the result | 🌐 🏆 ✅ |     |     |
| [no-hallucinated-local-imports](docs/rules/no-hallucinated-local-imports.md)       | Disallow static relative imports and re-exports that reference missing local modules or named exports                           | 🌐       |     |     |
| [no-hallucinated-package-imports](docs/rules/no-hallucinated-package-imports.md)   | Disallow package imports and re-exports whose package root is not declared in the relevant package manifest                     | 🌐       |     |     |
...
| [no-known-value-widening](docs/rules/no-known-value-widening.md)                   | Disallow broad explicit types that discard known value evidence                                                                 | 🌐 ✅ ⌨️ |     |     |
...
| [no-object-parameters](docs/rules/no-object-parameters.md)                         | Disallow broad object types in function parameters                                                                              | 🌐 ✅ ⌨️ |     |     |
| [no-redundant-comments](docs/rules/no-redundant-comments.md)                       | Disallow comments that merely narrate the next line of code without adding intent                                               | 🌐 🧹 ✅ |     |     |
...
| [no-type-system-bypass](docs/rules/no-type-system-bypass.md)                       | Disallow broad TypeScript escape hatches that hide type errors instead of fixing them                                           | 🌐 ✅ ⌨️ |     |     |
| [no-unbounded-promise-all](docs/rules/no-unbounded-promise-all.md)                 | Disallow Promise.all fan-out over arbitrary collections without an explicit concurrency bound                                   | 🌐       |     |     |
| [no-unknown-parameters](docs/rules/no-unknown-parameters.md)                       | Disallow explicit unknown function parameters except error-cause enrichment                                                     | 🌐 ✅ ⌨️ |     |     |
| [no-unknown-returns](docs/rules/no-unknown-returns.md)                             | Disallow function return contracts that expose unknown                                                                          | 🌐 ✅ ⌨️ |     |     |
| [no-unknown-type-aliases](docs/rules/no-unknown-type-aliases.md)                   | Disallow named type aliases that conceal unknown                                                                                | 🌐 ✅ ⌨️ |     |     |
| [no-unsafe-array-access](docs/rules/no-unsafe-array-access.md)                     | Require a non-empty array guard before reading the first or last element                                                        | 🌐 🏆 ✅ |     |     |
| [no-unsafe-dictionary-type](docs/rules/no-unsafe-dictionary-type.md)               | Disallow dictionary contracts whose values use unsafe escape-hatch types                                                        | 🌐 ✅ ⌨️ |     |     |
| [no-weak-randomness-for-secrets](docs/rules/no-weak-randomness-for-secrets.md)     | Disallow weak or predictable randomness when creating security-sensitive values                                                 | 🌐 🏆 ✅ |     |     |
| [no-widen-then-assert](docs/rules/no-widen-then-assert.md)                         | Disallow declaring a variable's type wider than its value, then asserting it back to the narrow type                            | 🌐 ✅ ⌨️ |     |     |
...
| [prefer-nullish-coalescing](docs/rules/prefer-nullish-coalescing.md)               | Prefer nullish coalescing over logical OR when providing default values                                                         | 🌐 ✅ 🎨 |     | 💡  |
